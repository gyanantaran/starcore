use eframe::egui::{CentralPanel, Color32, Pos2, Stroke, ViewportBuilder};
use eframe::{run_simple_native, NativeOptions};
...
fn main() -> eframe::Result {
    let mut myapp: app::App = App::default();
    ...
    run_simple_native("Name", options, move |ctx, _frame| {
        CentralPanel::default().show(ctx, |ui| {
            ...
            for a_body in myapp.simulation.bodies.iter_mut() {
                ...
                let my_stroke = Stroke {
                    width: 0.5,
                    color: Color32::WHITE,
                };
                painter.circle(Pos2 { x, y }, 2.0, color, my_stroke);
            }
        });
    })
}
